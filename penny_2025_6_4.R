...
# adding inflation under sr0 rounding (as fractions, not %). Used to determine the p.values for this ratio
d7.df$sr0_inf = d7.df$sr0 / (100*d7.df$amnt)

# adding inflation under ur rounding (as fractions, not %). Used to determine the p.values for this ratio
d7.df$ur0_inf = d7.df$ur0 / (d7.df$amnt)
#
sample_n(d7.df, 30)
...
# inflation results (stats): symmetric rounding (in fractions)
...
# row labels
#(var0.vec = c("Average rounding (number of pennies)", "Median rounding (number of pennies)", "p-value (rounding)", "Average inflationary consequence (%)", "Median inflationary consequence (%)", "p-value (inflation)"))# old version with average and median
(var0.vec = c("Average rounding (number of pennies)", "Median rounding (number of pennies)", "p-value (rounding)", "Inflationary impact (%)", "p-value (inflation)"))

# All data column sr0
(sr0_all_data.vec = c(avg_sr0_all, med_sr0_all, p_sr0_all, inf_sr0_all, p_sr0_inf_all))
#
# non_0_non_5 column sr0
(sr0_non_0_non_5_data.vec = c(avg_sr0_non_0_non_5, med_sr0_non_0_non_5, p_sr0_non_0_non_5, inf_sr0_non_0_non_5, p_sr0_inf_non_0_non_5))
#
# All data column ur0
(ur0_all_data.vec = c(avg_ur0_all, med_ur0_all, p_ur0_all, inf_ur0_all, p_ur0_inf_all))
#
# non_0_non_5 column ur0
(ur0_non_0_non_5_data.vec = c(avg_ur0_non_0_non_5, med_ur0_non_0_non_5, p_ur0_non_0_non_5, inf_ur0_non_0_non_5, p_ur0_inf_non_0_non_5))
...
dim(penny.df)
(digitm = matrix(c(rep(3,6), rep(3,6), rep(4,6), rep(3,6), rep(3,6), rep(4,6)), nrow = 5, ncol = 6, byrow = T))
#
print(xtable(penny.df, digits = digitm), include.rownames = F, hline.after = c(0,3))
...
# adding inflation under sr5 rounding (in %)
d8.df$sr5_inf = d8.df$sr5 / (d8.df$amnt)

# adding inflation under ur rounding
d8.df$ur5_inf = d8.df$ur5 / (d8.df$amnt)
#
sample_n(d8.df, 30)
...
# inflation results (stats): symmetric rounding (%)
...
# inflation results (stats): upward rounding (%)
...
# row labels
#(var5.vec = c("Average rounding (number of pennies)", "Median rounding (number of pennies)", "p-value (rounding)", "Average inflationary consequence (%)", "Median inflationary consequence (%)", "p-value (inflation)"))# old version, now inflation redefined
(var5.vec = c("Average rounding (number of pennies)", "Median rounding (number of pennies)", "p-value (rounding)", "Inflationary impact (%)", "p-value (inflation)"))

# All data column sr5
(sr5_all_data.vec = c(avg_sr5_all, med_sr5_all, p_sr5_all, inf_sr5_all, p_sr5_inf_all))
#
# non_0 column sr5
(sr5_non_0_data.vec = c(avg_sr5_non_0, med_sr5_non_0, p_sr5_non_0, inf_sr5_non_0, p_sr5_inf_non_0))
#
# All data column ur5
(ur5_all_data.vec = c(avg_ur5_all, med_ur5_all, p_ur5_all, inf_ur5_all, p_ur5_inf_all))
#
# non_0_non_5 column ur5
(ur5_non_0_data.vec = c(avg_ur5_non_0, med_ur5_non_0, p_ur5_non_0, inf_ur5_non_0, p_ur5_inf_non_0))
...
# below, create matrix w\ 1 extra column to indicate number of digits for each row
#
print(xtable(nickel.df, digits = digitm), include.rownames = F, hline.after = c(0, 3))
...
(nickel_inf_all_avg_sr.vec = as.vector(d8.df %>% group_by(merch) %>% summarise(sum(sr5_inf)/sum(amnt))))
...
#
(nickel_inf_all_avg_ur.vec = as.vector(d8.df %>% group_by(merch) %>% summarise(sum(ur5_inf)/sum(amnt))))
# this creates a list, so make it a vector
...
d9_non_0.df = d8_non_0.df %>% group_by(merch) %>% filter(n() > 9) 
...
(nickel_inf_non_0_avg_sr.vec = as.vector(d9_non_0.df %>% group_by(merch) %>% summarise(sum(sr5_inf)/sum(amnt))))
str(nickel_inf_non_0_avg_sr.vec)
# this creates a list, so make it a vector
(temp1.vec = unlist(nickel_inf_non_0_avg_sr.vec, use.names = F))
# it doubled the size so cut the vector
(temp2.vec = temp1.vec[7:12])
...
(nickel_inf_non_0_avg_ur.vec = as.vector(d9_non_0.df %>% group_by(merch) %>% summarise(sum(ur5_inf)/sum(amnt))))
# this creates a list, so make it a vector
(temp3.vec = (unlist(nickel_inf_non_0_avg_ur.vec , use.names = F)))
# it doubled the size so cut the vector
str(temp3.vec)
(temp4.vec = temp3.vec[7:12])
...
(merch_n_non_0.vec = temp9.vec[7:12])
...
(digits_nickel_inf_non_0 = matrix(c(0,0,0,2,2,4,0,4,0), nrow = 6, ncol = 9, byrow = T))
#
