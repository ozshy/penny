# Code for round_v?.tex titled: "Penny Elimination and Inflation"
...
library(texreg) # for displaying multinomial coefficients. Works with mfx.
#library(huxtable)#displays multiple regressions as table (then, use xtable to convert it to LaTeX). => advantage, since the table can be edited in R before xtable to LaTeX. => output does not run in LaTeX
#library(nnet)# multinomial regressions
#library(haven)# to handle haven labelled when data is converted from other software e.g. Stata
...
# focus on amount and merchant type only
trans2024_3.df = subset(trans2024_2.df, select = c(merch,amnt))
dim(trans2024_3.df)# number of in-person cash payments

# add column with the penny cent components (0 to 99), call it "decimal"
trans2024_4.df = as.data.frame(trans2024_3.df)
...
# simplify data frame name and deleting payments with 0 amount
nrow(subset(trans2024_5.df, amnt==0))
nrow(subset(trans2024_5.df, is.na(amnt)))
d5.df = subset(trans2024_5.df, amnt != 0)
dim(d5.df)
dim(trans2024_5.df)
nrow(trans2024_5.df) - nrow(d5.df)
...
var_list.vec
# convert to LaTeX
(texreg.latex = texreg(list(round_logit1, round_logit2), digits = 3, custom.model.names = c("All payments", "Payments $100 and lower"), custom.coef.names = var_list.vec))
